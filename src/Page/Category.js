...
const Category = () => {
    ...
    useEffect(() => {
      const applyFilters = () => {
        let filteredData = products.filter(product => product.categoryId === categoryId);
        ...
        <div className='flex  justify-center items-center gap-6 overflow-hidden'>
            {
                filterProduct.map((product)=>(
                    <ProductCard product = {product} onClick={() => handleAddToCart(product)}/>
                ))
            }
        </div>

       <div className=' self-start '>
       <Link to="/checkout"> <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="currentColor" class="bi bi-cart-check-fill" viewBox="0 0 16 16">
